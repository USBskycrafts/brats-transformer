...
class TestMaskGit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 创建测试数据
        self.input_indices = torch.randint(0, self.vocab_size, (self.batch_size, self.seq_len))
        self.target_indices = torch.randint(0, self.vocab_size, (self.batch_size, self.num_tokens))
        self.conditions = torch.randint(0, self.vocab_size, (self.batch_size, self.seq_len))

    def test_forward_basic(self):
        """测试forward函数的基本功能"""
        # 设置mock transformer的返回值
        logits_shape = (self.batch_size, self.seq_len + self.num_tokens, self.vocab_size)
    ...
    def test_generate_basic(self):
        ...
        for step_indices in generated_indices:
            self.assertEqual(step_indices.shape, (self.batch_size, self.num_tokens))
    ...
    def test_generate_different_temperatures(self):
        ...
        for temperature in test_temperatures:
            with self.subTest(temperature=temperature):
                ...
                self.assertEqual(final_indices.shape, (self.batch_size, self.num_tokens))
    ...
    def test_create_random_mask(self):
        ...
        for ratio in [0.0, 0.3, 0.7, 1.0]:
            with self.subTest(ratio=ratio):
                mask = self.maskgit._create_random_mask(shape, ratio, self.device)
    ...
    def test_compute_unmask_count_cosine(self):
        ...
        current_mask = torch.ones((self.batch_size, self.num_tokens), dtype=torch.bool)
    ...
    def test_device_consistency(self):
        ...
        if torch.cuda.is_available():
            ...
            conditions_gpu = self.conditions.to(device)
            
            logits_shape = (self.batch_size, self.seq_len + self.num_tokens, self.vocab_size)
            mock_logits = torch.randn(logits_shape).to(device)
            self.mock_transformer.return_value = mock_logits
            
            generated_indices = self.maskgit.generate(
                num_tokens=self.num_tokens,
                conditions=conditions_gpu,
                device=device,
                steps=4,
                temperature=1.0
            )
            ...
            # 验证结果在正确的设备上
            for step_indices in generated_indices:
                self.assertEqual(step_indices.device, device)
    ...
    def test_batch_consistency(self):
        ...
        for batch_size in [1, 3, 5]:
            with self.subTest(batch_size=batch_size):
                conditions = torch.randint(0, self.vocab_size, (batch_size, self.seq_len))
                
                logits_shape = (batch_size, self.seq_len + self.num_tokens, self.vocab_size)
                mock_logits = torch.randn(logits_shape)
                self.mock_transformer.return_value = mock_logits
